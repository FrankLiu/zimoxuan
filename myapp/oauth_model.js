...
model.createClient = function(clientId, clientSecret, redirectUri){
	console.log('createClient (clientId: ' + clientId + ', clientSecret: ' + clientSecret + ')');
	var model = new OAuthClientsModel();
	model.clientId = clientId;
	model.clientSecret = clientSecret;
	model.redirectUri = redirectUri;
	model.save(function(err){
		if(err) console.log('error on creating client!' + e);
	});
};
...
var authorizedClientIds = ['s6BhdRkqt3', 'toto', 'testclient'];
...
model.createUser = function(username, password, firstName, lastName, email){
	...
	user.save(function(err){
		if(err) console.log('error on creating user!' + e);
	});
}
